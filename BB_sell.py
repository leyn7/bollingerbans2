...
def analizar_señales_venta(
    ...
    log_prefix = f"BB_sell ({symbol})"
    if verbose:
        logger.info(f"{log_prefix}: Iniciando lógica de detección de señal de VENTA.")
    ...
    try:
        ...
        bbu_orig_main = Decimal(str(bands_main_dict['BBU_orig'])) 
        bbu_new_main = Decimal(str(bands_main_dict['BBU_new']))   # Este será el entry_price_target para VENTA
    ...
    if verbose: logger.debug(f"{log_prefix}: Bandas {interval_principal_str} (contexto): BBU_orig={bbu_orig_main}, BBM_orig={bbm_orig_main}, BBU_new(target_entry)={bbu_new_main}")
    ...
    # 4. Verificar las Nuevas Condiciones para Venta
    signal_detected = False
    # Condición 1: BBU_orig_5m < BBM_15m
    pre_condition_met = bbu_orig_main < bbm_sl_ref
    # Condición 2: precio_actual_1m > BBM_orig_5m
    price_trigger_met = current_price_trigger_interval > bbm_orig_main

    if verbose:
        logger.debug(f"{log_prefix}: Chequeo VENTA:")
        logger.debug(f"  Pre-condición (BBU_orig_{interval_principal_str} ({bbu_orig_main}) < BBM_{interval_sl_ref_str} ({bbm_sl_ref})) = {pre_condition_met}")
    ...
    if pre_condition_met and price_trigger_met:
        logger.info(f"{log_prefix}: ¡CONDICIONES CRUDAS DE SEÑAL DE VENTA DETECTADAS para {symbol} @ {current_price_trigger_interval} (vela {interval_trigger_str})!")
    ...
    if signal_detected:
        raw_signal_components = {
            "signal_detected": True,
            "entry_price_target": bbu_new_main, # Este es el precio de entrada objetivo para VENTA
            ...
            "bbm_orig_main_tf": bbm_orig_main,
            "bbu_orig_main_tf": bbu_orig_main 
        }
        # Ya NO se llama a validate_and_calculate_trade_params aquí.
        
        # --- Lógica de Visualización (Verbose Mode) ---
        if verbose:
            logger.info(f"{log_prefix}: Mostrando contexto de datos para señal VENTA detectada (antes de validación final).")
            # (El código de display que tenías puede ir aquí)
            # Ejemplo simplificado:
            df_5m_display_with_bands = None # Renombrado para claridad
            ...
            if df_5m_display_with_bands is not None and not df_5m_display_with_bands.empty:
                ...
                print(f"\n--- ({log_prefix}) Contexto Señal VENTA {interval_principal_str} ({symbol}) ---")
        ...
        return raw_signal_components

    # Si no se detectó señal
    if verbose:
        logger.debug(f"{log_prefix}: No se detectaron condiciones de señal de VENTA en este ciclo.")
    
    if verbose:
        # ... (código de display si no hubo señal, similar al de arriba)
        print("-" * 180)
